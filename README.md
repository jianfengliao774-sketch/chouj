...
这些新合约尚待用户签名部署及核验。首页使用独立的待开放状态，允许连接真实钱包只读余额，禁用授权、购买与退款；不能通过修改后端开售标记启用交易。新合约部署、运行代码及固定规则核验、VRF 消费者注册、专用自动执行器和一次性容器授权完成后，还须发布对应的交易集成。原合约入口保留在 `/legacy.html`，原规则继续适用，不能用原合约地址代替新档位。
...
奖金卡展示 BEM 奖金和独立 BEM/USDT、BEM/WBNB 市场报价换算；后端按地址核验交易对、每 60 秒缓存更新，读取失败明确停止展示实时换算。报价只用于参考展示，不参与任何票价或结算计算。中奖播报和 `/burns.html` 仅从经过 12 个区块确认、回执及主链检查的记录生成交易链接；新档位尚未部署时不生成示例中奖或销毁记录。
