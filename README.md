...
首页：<http://127.0.0.1:8788/>，后台：<http://127.0.0.1:8788/admin.html>。若端口被占用，可用 `pnpm start --port 8789`。服务只监听本机地址，尚未配置公网域名或 HTTPS。

网站启动时核对链 ID 和已部署代码哈希。它通过只读 RPC 读取主网，服务端不能签名或广播交易。管理员使用当前 2075 持有人钱包签名登录，现有后台仅查看状态。主网页面不会自动使用测试钱包。
