...
`/deploy-formal.html` 用于新 V3 的 10 / 50 / 100 BEM 独立部署。部署钱包固定为已核验的 `0x304F06903324B8056cB1ED627144EfB2C34df3a8`；授权启动和收款使用 13061 容器，2075 仅负责计算。每档仍需用户单独签署部署交易，再核验地址、添加 VRF 消费者、启动并登记上线。页面不会把已有 V2 地址当成 V3，也不会自动重发交易。
