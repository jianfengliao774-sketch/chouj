# Tapeout 芯火夺宝 / Tapeout SparkDraw

页面与内部机制的分工、分支和本地启动见 [协作说明](COLLABORATION.md)。

本次更新：所有访客可查看封盘、VRF、结算状态与目标倒计时；中奖号码和钱包随链上结果更新。`/#mine` 提供按钱包和期号查询的服务器参与记录；购买入口已合并为“授权并购买”，已有足够额度时跳过授权。
...
`/deploy-formal.html` 用于新 V3 的 10 / 50 / 100 BEM 独立部署。部署钱包固定为已核验的 `0x7674fa446D42b1f7f150DC5e678cc525d275Ea53`；授权启动和收款使用 13061 容器，2075 仅负责计算。每档仍需用户单独签署部署交易，再核验地址、添加 VRF 消费者、启动并登记上线。页面不会把已有 V2 地址当成 V3，也不会自动重发交易。

V3 支持按链上实际剩余份数和地址额度部分成交：申请 1,000 份而只剩 800 份，只扣 800 份金额，另 200 份金额留在钱包。确认回执后显示中英文成交提示和交易哈希。当前 1 BEM 测试仍使用已部署的 V2，不能通过改网页获得 V3 规则；新正式地址登记前正式购买入口保持禁用。详见 [V3 合约说明](contracts/production-v3/README.md)。

玩家链上查询按最多 25 项合并 HTTP 请求，只对暂时不可用的只读查询有限重试。签名和交易广播不进入重试通道。页面读取失败会明确提示重试，不据此显示合约“未启动”。服务器分别限制 API、RPC 请求，静态资源不消耗两者预算；上游查询保留有界并发和队列，防止大量访客造成无界积压。容量仍取决于 RPC 上游和服务器资源。
...
授权与购买采用快速回执模式：一份已核对交易身份、成功状态和事件的上链回执即可更新结果并解除等待；提交前只做一轮新鲜状态核验及一次模拟、Gas 估算。有待处理订单时约每 2 秒查询，空闲页面保持较低读取频率。退款、结算、部署和公共历史索引仍使用各自的确认门槛。未连接钱包时，点击购买区按钮可连接并保留选号，连接后不会自动提交金融交易。
