...
function setup(){let ctx={account,key:'same'},price=50_000_000n,receipt=null,tx=null;const storage=new Map(),sent=[];
  ...
  const manager=createSparkDrawTransactions({checkSales:()=>{},rpc,wallet:()=>wallet,context:()=>ctx,locks:{request:async(k,o,fn)=>fn({})},storage:{getItem:k=>storage.get(k)||null,setItem:(k,v)=>storage.set(k,v),removeItem:k=>storage.delete(k)}});
