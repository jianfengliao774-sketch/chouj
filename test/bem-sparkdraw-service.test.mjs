...
test('V5 service exposes only the five new pools, routes old pages away and rejects old RPC calls',async t=>{
  ...
  const service=await createSparkDrawService({rpc,directory,credential,verify:false,staticRoot:path.resolve('bem-production-site/web/public'),origin:'http://127.0.0.1:18991'});await new Promise(r=>service.server.listen(18991,'127.0.0.1',r));
