...
import {AUTHORIZATION_MS,authorizedControl} from './sparkdraw-authorization.mjs';
...
export async function createSparkDrawVault({directory,secret,statusFile,keyStore=null,controlFile=path.join(directory,'automation-control.json'),now=()=>Date.now()}){
  ...
  const atomic=async(file,data)=>{const mode=file===controlFile?0o640:0o600;await fs.writeFile(file+'.tmp',JSON.stringify(data),{mode});await fs.chmod(file+'.tmp',mode);await fs.rename(file+'.tmp',file);};
  async function status(){let worker=null;try{worker=JSON.parse(await fs.readFile(statusFile,'utf8'));}catch{}const fresh=worker&&now()-Date.parse(worker.updatedAt)<45000;
    const savedKey=keyStore?await keyStore.status():null;let control=null;try{control=JSON.parse(await fs.readFile(controlFile,'utf8'));}catch{}
    return{configured:!!secret,enrolled:state.enrolled,keyImportAvailable:!!keyStore,savedKey,authorizationActive:authorizedControl(control,now()),authorizationExpiresAt:control?.authorizationExpiresAt||0,worker:worker?{...worker,online:!!fresh}:null};}
  ...
  async function authorize({code,enroll=false,username,purpose='control'}){
    ...
    const counterKey=purpose==='login'?'lastLoginCounter':'lastCounter';
    const counter=matchTotp(secret,String(code||''),now(),state[counterKey]??-1);
    if(counter===null){attempts.push(now());throw Object.assign(Error('验证码无效、过期或已经使用。'),{authStatus:403});}
    state={...state,version:1,enrolled:true,[counterKey]:counter,username};await atomic(file,state);attempts=[];
  }
  function update({code,enabled,enroll=false,username}){const run=queue.then(async()=>{
    ...
    await authorize({code,enroll,username});await atomic(controlFile,{enabled,authorizationExpiresAt:enabled?now()+AUTHORIZATION_MS:0,updatedAt:new Date(now()).toISOString(),by:username});return status();
  ...
  function importKey({code,privateKey,expectedAddress,username}){const run=queue.then(async()=>{
    ...
    await atomic(controlFile,{enabled:false,resumeAfterKeyId:saved.id,authorizationExpiresAt:now()+AUTHORIZATION_MS,updatedAt:new Date(now()).toISOString(),by:username});
    return status();
  });queue=run.catch(()=>{});return run;}
  function verifyLogin({code,username}){const run=queue.then(async()=>{
    if(!secret)throw Object.assign(Error('谷歌验证服务尚未配置。'),{authStatus:503});
    // Initial password-authenticated setup is allowed only until the authenticator is enrolled.
    if(!state.enrolled)return;
    await authorize({code,username,purpose:'login'});
  });queue=run.catch(()=>{});return run;}
  return{status,setup,update,importKey,verifyLogin};
}
