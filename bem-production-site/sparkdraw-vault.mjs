...
export async function createSparkDrawVault({directory,secret,statusFile,controlFile=path.join(directory,'automation-control.json'),now=()=>Date.now()}){
  ...
  async function status(){let worker=null;try{worker=JSON.parse(await fs.readFile(statusFile,'utf8'));}catch{}const fresh=worker&&now()-Date.parse(worker.updatedAt)<45000;
    return{configured:!!secret,enrolled:state.enrolled,worker:worker?{...worker,online:!!fresh}:null};}
  ...
  function update({code,enabled,enroll=false,username}){const run=queue.then(async()=>{
    if(!secret||typeof enabled!=='boolean')throw Object.assign(Error('保险箱尚未配置。'),{authStatus:409});
    ...
    state={version:1,enrolled:true,lastCounter:counter,username};await atomic(file,state);await atomic(controlFile,{enabled,updatedAt:new Date(now()).toISOString(),by:username});attempts=[];return status();
  });queue=run.catch(()=>{});return run;}
  return{status,setup,update};
}
