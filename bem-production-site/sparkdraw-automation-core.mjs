...
export function validateAutomationTransaction(raw,address){
  ...
  if(!p||getAddress(tx.from)!==getAddress(address)||tx.chainId!==56n||tx.value!==0n||tx.type!==0||tx.gasLimit<=0n||tx.gasLimit>AUTOMATION_LIMITS.gasLimit||!tx.gasPrice||tx.gasPrice>AUTOMATION_LIMITS.gasPrice||tx.gasPrice*tx.gasLimit>AUTOMATION_LIMITS.fee)throw Error('INVALID_AUTOMATION_TRANSACTION');
