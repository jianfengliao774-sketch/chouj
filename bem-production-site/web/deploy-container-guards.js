...
export const DEPLOY_MODES = Object.freeze({
  production: Object.freeze({ name: '正式合约 · 每期 100 BEM', contractName: 'Bem2075Raffle13061BSC', pool: '10000000000', ticketPrice: '1000000', tickets: '10000', maxPerPurchase: '1000', blackhole: '400000000', organizer: '100000000', winner: '9500000000' }),
  pool10: Object.freeze({ name: '正式合约 · 每期 10 BEM', contractName: 'Bem2075Raffle13061Pool10BSC', pool: '1000000000', ticketPrice: '100000', tickets: '10000', maxPerPurchase: '1000', blackhole: '40000000', organizer: '10000000', winner: '950000000' }),
  pool50: Object.freeze({ name: '正式合约 · 每期 50 BEM', contractName: 'Bem2075Raffle13061Pool50BSC', pool: '5000000000', ticketPrice: '500000', tickets: '10000', maxPerPurchase: '1000', blackhole: '200000000', organizer: '50000000', winner: '4750000000' }),
  test: Object.freeze({ name: '内部测试合约 · 每期 1 BEM', contractName: 'Bem2075Raffle13061Test1BSC', pool: '100000000', ticketPrice: '10000', tickets: '10000', maxPerPurchase: '1000', blackhole: '4000000', organizer: '1000000', winner: '95000000' }),
});
...
export function deploymentMode(mode) { need(Object.hasOwn(DEPLOY_MODES, mode), '请先选择小额测试合约或正式合约。'); return DEPLOY_MODES[mode]; }
...
export function validateDeploymentArtifact(artifact, mode) {
  ...
  need(artifact.testOnly === (mode === 'test'), '部署文件的测试标记不匹配。');
  ...
  need(rules && String(rules.poolBaseUnits) === selected.pool && String(rules.ticketPriceBaseUnits) === selected.ticketPrice &&
...
export function assertContainerBindings(binding) {
  ...
  for (const [name, nft, tokenId, account] of [
    ['authorization', f.processor, f.processorId, f.authorizationContainer],
...
export const ADDRESS_READBACK = Object.freeze({ bem: DEPLOY_FIXED.bem, organizer: DEPLOY_FIXED.revenueContainer,
  CONTAINER: DEPLOY_FIXED.authorizationContainer, AUTHORIZATION_NFT: DEPLOY_FIXED.processor, CIRCUITS: DEPLOY_FIXED.processor,
...
export function expectedReadback(mode, deploymentBlock) {
  ...
  return { ...ADDRESS_READBACK, AUTHORIZATION_TOKEN_ID: '2075', CIRCUIT_ID: '2075', REVENUE_TOKEN_ID: '13061',
    ...
    REFUND_CLAIM_WINDOW: '86400', MAX_TICKETS_PER_ADDRESS: '5000',
    drawWindow: '3600', NEXT_ROUND_DELAY: '60', keyHash: f.keyHash, CIRCUIT_HASH: f.circuitHash,
    sourceVerifiedAtBlock: String(deploymentBlock), seriesAuthorized: false, currentRoundId: '1', totalLiability: '0', ...(mode === 'test' ? { TEST_ONLY: true } : {}) };
}
...
export function assertDeploymentTransaction({ transaction, receipt, block, latestBlock, account, data, nonce }) {
  ...
  need(BigInt(transaction.nonce) === BigInt(nonce), '交易 nonce 与本次部署记录不同。');
  need(sameAddress(receipt.transactionHash, transaction.hash) && sameAddress(receipt.from, account) && receipt.to == null &&
    ...
    BigInt(receipt.blockNumber) === BigInt(transaction.blockNumber) && BigInt(block.number) === BigInt(receipt.blockNumber) &&
    Array.isArray(block.transactions) && block.transactions.some(hash => sameAddress(hash, transaction.hash)), '交易回执与主链区块不一致。');
  need(BigInt(receipt.status) === 1n, '该部署交易已失败，未创建合约。');
  need(sameAddress(receipt.contractAddress, getCreateAddress({ from: account, nonce })), '部署合约地址不符合创建交易。');
  const confirmations = Number(BigInt(latestBlock) - BigInt(receipt.blockNumber) + 1n);
...
export function assertCreationTransaction(transaction, account, data) {
  need(transaction && /^0x[0-9a-f]{64}$/i.test(transaction.hash) && BigInt(transaction.chainId) === 56n &&
    sameAddress(transaction.from, account) && transaction.to == null &&
    (transaction.input ?? transaction.data)?.toLowerCase() === data.toLowerCase() && BigInt(transaction.value) === 0n,
    '交易不是当前钱包所选档位的合约创建交易。');
}
export function assertUnsuccessfulDeployment({ transaction, receipt, block, latestBlock, account, data, nonce, originalTransaction, senderCode, deployedCode }) {
  ...
  need(BigInt(transaction.chainId) === 56n && sameAddress(transaction.from, account) && BigInt(transaction.nonce) === BigInt(nonce), '交易身份或序号不匹配。');
  need(sameAddress(receipt.transactionHash, transaction.hash) && sameAddress(receipt.from, account) &&
    ...
    BigInt(latestBlock) - BigInt(receipt.blockNumber) + 1n >= 12n, '交易尚未得到足够主链确认，不能释放部署锁。');
  ...
  need(BigInt(originalTransaction.nonce) === BigInt(transaction.nonce) && !sameAddress(originalTransaction.hash, transaction.hash) &&
    BigInt(receipt.status) === 1n && sameAddress(transaction.to, account) && sameAddress(receipt.to, account) &&
    BigInt(transaction.value) === 0n && (transaction.input ?? transaction.data) === '0x' &&
...
export function assertDeploymentIntent({ mode, account, epoch, estimate }, current, now = Date.now()) {
  ...
  need(estimate && estimate.expiresAt > now && estimate.createdAt <= now && estimate.expiresAt - estimate.createdAt <= 90000,
    '费用核对已过期，请重新估算。');
}
