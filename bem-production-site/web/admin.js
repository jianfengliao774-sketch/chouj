...
function render(data) {
  $('login-panel').hidden = true; $('admin-content').hidden = false; $('admin-account').textContent = `当前管理员：${data.session.username}`;
...
async function refresh({ initial = false } = {}) {
  try { render(await api('/api/admin/status')); }
  catch (error) {
    if (error.status === 401) {
      $('login-panel').hidden = false; $('admin-content').hidden = true;
      notice(initial ? '请输入管理员账号和密码。' : '登录已过期，请重新登录。', !initial);
...
$('logout').addEventListener('click', async () => {
  ...
  try { await api('/api/admin/logout', {}); $('admin-content').hidden = true; $('login-panel').hidden = false; $('admin-password').value = ''; notice('已退出管理后台。'); }
