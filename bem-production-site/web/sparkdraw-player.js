...
import {SALES_POOL_IDS,DEFAULT_POOL_ID,poolSalesEnabled} from './sparkdraw-sales-policy.js';
...
function bind(p){if(bound.has(p))return;bound.add(p);
  ...
  p.on?.('disconnect',()=>{if(wallet!==p)return;revision++;account=null;chain=null;balance=null;heldContext=null;allowance=0n;walletSession.cancel();render();refreshRecords();note(t('钱包已断开，请重新连接。','Wallet disconnected. Connect again to continue.'));});
...
function render(){
  ...
  $('connect-wallet').textContent=connectingWallet?t('等待钱包确认…','Waiting for wallet…'):account?t('切换钱包','Switch wallet'):t('连接钱包','Connect wallet');
  $('connect-wallet').disabled=connectingWallet;
  ...
  $('launch-status').textContent=t('正式场已开放：5 / 10 / 50 BEM','Live pools: 5 / 10 / 50 BEM');
  $('sale-note').textContent=p.salesEnabled?t('正式场已开放，首笔购买开始 24 小时募集。','This pool is open. The first purchase starts the 24-hour funding period.'):t('本场已关闭新购买，仍可查看记录、领取奖金和退款。','New purchases are closed. Records, prize claims and refunds remain available.');
  ...
  renderDraw();renderPending();renderSelector();renderRefund();renderHistory();schedulePurchasePreparation();
}
...
function renderPending(){
  const rows=manager.pendings,history=manager.history.slice(-5).reverse(),key=JSON.stringify([account,rows,history,getLanguage(),manager.busy]);
...
async function preparePurchase(){
  ...
  if(context().key!==key)throw Object.assign(Error('CONTEXT_CHANGED'),{code:'CONTEXT_CHANGED'});
  const needsApproval=al[0]<BigInt(filled)*p.ticketPrice;
  const chosen=needsApproval?null:s.tickets||randomUnsoldTickets(stock[0],filled);
...
async function buy(){
  if(!poolSalesEnabled(pool)){failure({code:'POOL_SALES_CLOSED'});return;}
...
function claimButton(r,id){
  const result=prizeResult(r,account,chainNow());
  if(result!=='claimable'){
    const labels={lost:t('本期未中奖，感谢参与','No win this round. Thank you for participating.'),claimed:t('奖金已领取','Prize claimed'),burned:t('奖金已销毁','Prize burned'),expired:t('领取期已结束','Claim window expired')};
    const message=el('p',labels[result]||'');message.hidden=!result;return message;
  }
  return button(t('领取奖金','Claim prize'),()=>{
    if(prizeResult(r,account,chainNow())!=='claimable')return refreshRecords();
    return action(id,'claimPrizes',[[r.roundId],account]);
  });
}
...
async function refreshRecords(){if(document.hidden)return;const version=++recordsVersion,id=pool,queriedAccount=account;const jobs=[];
  if(tab==='proof')jobs.push(queryRecords('winners',{pool:id,page:historyPage,...(queriedAccount?{address:queriedAccount}:{})}).then(d=>{if(version!==recordsVersion||queriedAccount!==account)return;historyRows=d.rows;renderHistory();$('history-summary').textContent=t('已确认 {n} 条中奖记录','{n} confirmed wins',{n:d.total});$('history-page').textContent=`${d.page} / ${d.totalPages}`;$('history-prev').disabled=d.page<=1;$('history-next').disabled=d.page>=d.totalPages;}));
