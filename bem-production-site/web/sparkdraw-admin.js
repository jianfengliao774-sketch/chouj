    ...
    for(const r of p.rounds){const detail=node('details',''),summary=node('summary',`${r.displayRoundId||r.roundId} · ${r.sold} 份 · 状态 ${r.status}`);detail.append(summary);
      ...
      let loaded=false;detail.ontoggle=async()=>{if(!detail.open||loaded)return;try{const data=await api(`/api/admin/round?pool=${p.poolId}&round=${r.roundId}`),wallets=new Map();
        ...
        for(const[a,w]of wallets){const section=node('div','');section.className='scope-note';section.append(node('h3',a),node('p',`购买 ${w.tx.size} 次 · ${w.count} 份 · ${money(w.paid)} BEM`));
          for(const e of w.tx.values()){const row=node('p',`${formatSiteTime(e.timeUtc,'zh')} · ${e.args.count} 份 · ${money(e.args.paid)} BEM `);row.append(link(e.transactionHash));section.append(row);}detail.append(section);}loaded=true;
