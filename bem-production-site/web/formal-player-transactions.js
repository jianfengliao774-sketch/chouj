...
function validRecord(value) {
  ...
  const load = () => {
    try {
      ...
      const raw = storage.getItem(STORAGE_KEY); pending = raw == null ? null : validRecord(JSON.parse(raw)); storageError = false;
  ...
  async function checkPending() {
    ...
    checking = withLock(async () => {
      ...
      try {
        ...
        if (count >= BigInt(requiredConfirmations)) { if (persist(null, record.id)) { if (checked.status === 'confirmed' && checked.kind === 'buy') verifiedResults.add(checked); history.unshift(checked); if (history.length > 25) history.pop(); } }
