...
export function drawGuideHtml(html) {
  const file=read('./web/public/sparkdraw/deployed-contracts.json'), registry=JSON.parse(file);
  ...
  const entries=['verifier',...POOL_IDS].map(id=>{
    ...
    const title=id==='verifier'?text('共用随机数验证合约','Shared randomness verifier'):id+' BEM '+text(id==='0.1'?'测试场':'场次',id==='0.1'?'test pool':'pool');
  ...
  const dependencies=[
    ...
    row('部署证据文件 · SHA-256','Deployment evidence file · SHA-256','<code>'+createHash('sha256').update(file).digest('hex')+'</code><a href="/sparkdraw/deployed-contracts.json" download>'+text('下载六份部署记录','Download all six deployment records')+'</a>')
  ].join('');
  const fees=POOL_IDS.map(id=>'<tr><td>'+id+' BEM</td><td>1%</td><td>'+profile(id).burnPercent+'%</td><td>'+(99-profile(id).burnPercent)+'%</td></tr>').join('');
