...
import "./BemSelectableRaffle13061BSC.sol";

interface ISeries13061ContainerOpener {
...
interface ISeries13061ContainerAccount {
    function token() external view returns (uint256 chainId, address tokenContract, uint256 tokenId);
}

/// @notice V2 candidate: TapeOut #13061's container receives 1% per settlement.
/// BEHEMOTH #2075 remains the sole processor and its container retains activation.
...
contract BemContainer13061SeriesBSC is BemSelectableRaffle13061BSC {
    ...
    ) BemSelectableRaffle13061BSC(REVENUE_CONTAINER, vrfSubscriptionId, confirmations, vrfCallbackGasLimit, poolBaseUnits) {
        if (authorizationNft != CIRCUITS || authorizationTokenId != CIRCUIT_ID
            || containerAddress != BEHEMOTH_2075_CONTAINER || OPENER.code.length == 0
            || ISeries13061ContainerOpener(OPENER).accountOf(authorizationNft, authorizationTokenId) != containerAddress) {
            revert InvalidContainer();
        }
        if (REVENUE_CONTAINER.code.length == 0
            || ISeries13061ContainerOpener(OPENER).accountOf(REVENUE_NFT, REVENUE_TOKEN_ID) != REVENUE_CONTAINER
            || !ISeries13061ContainerOpener(OPENER).isOpened(REVENUE_NFT, REVENUE_TOKEN_ID)) {
            revert InvalidContainer();
        }
        (uint256 revenueChain, address revenueNft, uint256 revenueId) = ISeries13061ContainerAccount(REVENUE_CONTAINER).token();
    ...
    function authorizeSeries() external nonReentrant {
        ...
        if (CONTAINER.code.length == 0
            || !ISeries13061ContainerOpener(OPENER).isOpened(AUTHORIZATION_NFT, AUTHORIZATION_TOKEN_ID)
            || ISeries13061ContainerOpener(OPENER).accountOf(AUTHORIZATION_NFT, AUTHORIZATION_TOKEN_ID) != CONTAINER) {
            revert InvalidContainer();
        }
        (uint256 chainId, address nft, uint256 tokenId) = ISeries13061ContainerAccount(CONTAINER).token();
