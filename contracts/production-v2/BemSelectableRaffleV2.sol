...
// Shared static V2 candidate. Release leaves pin one of four denominations.
// Container authorization and operating cadence belong to the series wrapper.

interface IV2SelectableRaffleToken {
...
interface IV2SelectableRaffleCircuitSource {
...
library V2SelectableRaffleVRFClient {
...
interface IV2SelectableRaffleVRFCoordinator {
    function requestRandomWords(V2SelectableRaffleVRFClient.RandomWordsRequest calldata request)
...
contract BemSelectableRaffleV2 {
    ...
    IV2SelectableRaffleToken public immutable bem;
    IV2SelectableRaffleVRFCoordinator public immutable coordinator;
    ...
    constructor(
        ...
        if (IV2SelectableRaffleToken(token).decimals() != 8) revert BadConfiguration();
        if (keccak256(CIRCUIT_RAW) != CIRCUIT_HASH) revert WrongCircuit();
        IV2SelectableRaffleCircuitSource source = IV2SelectableRaffleCircuitSource(CIRCUITS);
        ...
        bem = IV2SelectableRaffleToken(token);
        coordinator = IV2SelectableRaffleVRFCoordinator(vrfCoordinator);
    ...
    /// @notice Inputs are zero-based, strictly ascending and unique. Requested
    /// unsold tickets are reserved first; sold requests move forward to the next
    /// free ticket, wrapping from9999 to0. Events report the actual allocation.
    /// Invalid input, insufficient remaining supply or payment failure reverts all.
    ...
    function _beginTicketPurchase(uint256 expectedRoundId, uint32 count) private returns (uint256 roundId) {
        ...
        uint256 addressTotal = uint256(ticketsOf[roundId][msg.sender]) + count;
        if (addressTotal > MAX_TICKETS_PER_ADDRESS) revert AddressTicketLimitExceeded(addressTotal, MAX_TICKETS_PER_ADDRESS);
    ...
    function _finishTicketPurchase(uint256 roundId, uint32 count) private {
        ...
        _tokenCall(abi.encodeCall(IV2SelectableRaffleToken.transferFrom, (msg.sender, address(this), amount)));
    ...
    function _requestDraw(uint256 roundId) internal returns (uint256 requestId) {
        ...
        requestId = coordinator.requestRandomWords(V2SelectableRaffleVRFClient.RandomWordsRequest({
            keyHash: keyHash,
    ...
    function _payWinner(uint256 roundId, Round storage r, uint32 ticket) private {
        ...
        _tokenCall(abi.encodeCall(IV2SelectableRaffleToken.transfer, (BLACKHOLE, BLACKHOLE_AMOUNT)));
        ...
        _tokenCall(abi.encodeCall(IV2SelectableRaffleToken.transfer, (organizer, ORGANIZER_AMOUNT)));
        _tokenCall(abi.encodeCall(IV2SelectableRaffleToken.transfer, (winner, WINNER_AMOUNT)));
    ...
    function refund(uint256 roundId, address participant) external nonReentrant {
        ...
        totalLiability -= amount;
        _tokenCall(abi.encodeCall(IV2SelectableRaffleToken.transfer, (participant, amount)));
        _assertSolvent();
    ...
    function burnUnclaimed(uint256 roundId) external nonReentrant {
        ...
        uint256 beforeBalance = bem.balanceOf(address(this));
        uint256 deadBefore = bem.balanceOf(BLACKHOLE);
        _tokenCall(abi.encodeCall(IV2SelectableRaffleToken.transfer, (BLACKHOLE, amount)));
