...
// Offline V3 compilation only. No RPC, wallet, transaction or V1/V2 writes.
const root = fileURLToPath(new URL('../', import.meta.url));
const outputDir = join(root, 'outputs/bem-raffle-2075/production-v3');
const sha256 = value => createHash('sha256').update(value).digest('hex');
const preserved = JSON.parse(readFileSync(join(outputDir, 'preserved-v1-v2.sha256.json'), 'utf8'));
assert.equal(sha256(JSON.stringify(preserved.files)), '5e32b076506d2c447da57116a07d30a04476cc11df8a1b1fa1a3ab0c10a5d299', 'Frozen V1/V2 manifest changed');
...
function guardPreserved() {
  for (const [name, expected] of Object.entries(preserved.files)) {
    assert.equal(sha256(readFileSync(join(root, name))), expected, `Deployed V1/V2 file changed: ${name}`);
  }
}
guardPreserved();
const dependencies = [
  'contracts/production-v3/BemSelectableRaffleV3.sol',
  'contracts/production-v3/BemSelectableRaffle13061V3BSC.sol',
  'contracts/production-v3/BemOwnContainer13061SeriesBSC.sol',
];
const targets = [10, 50, 100].map(pool => {
  const contractName = `BemOwnContainer13061Pool${pool}BSC`;
  const poolBaseUnits = BigInt(pool) * 100000000n;
  return { contractName, sourceName: `contracts/production-v3/${contractName}.sol`, fixedRules: {
    ...
    blackholeBaseUnits: String(poolBaseUnits * 4n / 100n), organizerBaseUnits: String(poolBaseUnits / 100n),
    winnerBaseUnits: String(poolBaseUnits * 95n / 100n), fundingWindowSeconds: 86400, refundClaimWindowSeconds: 86400,
    partialFill: true,
  } };
});
...
const architecture = 'container13061-authorized-2075-computation';
const partialFill = {
...
for (const { sourceName, contractName, fixedRules } of targets) {
  ...
  const artifact = {
    schemaVersion: 1, contractVersion: 3, architecture, status: 'production-candidate-not-deployed',
  ...
  const buildInfo = {
    status: artifact.status, contractVersion: 3, architecture, contractName, sourceName, compilerVersion: artifact.compilerVersion,
  ...
  guardPreserved();
  console.log(JSON.stringify({ contractName, contractVersion: 3, runtimeBytecodeBytes: artifact.runtimeBytecodeBytes,
