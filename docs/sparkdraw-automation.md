...
登录后台，在「保存私钥并替换自动开奖钱包」输入私钥，页面先显示对应公开地址。核对地址后点击「验证、保存并自动开奖」，在独立弹窗输入谷歌验证码并确认。弹窗取消不提交私钥；提交期间禁止重复操作，退出登录后清空私钥与验证码。当前服务须已暂停、无待确认交易；切换完成后服务自动继续。指定钱包存在链上待确认交易时，等待处理完成后再启用。新钱包须有 BNB 支付 Gas。
