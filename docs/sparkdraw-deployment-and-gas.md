...
All five pools were round 1, unstarted, zero sold at verification. First purchase starts funding. The old 1 BEM VRF test has a separate address and remains separate. Deployment registration does **not** activate the V5 player adapter, history indexer or keeper. These still need integration; do not inject these addresses into the legacy VRF profiles.
...
The active test, legacy and formal transaction modules call this policy. Any new V5 transaction adapter must call it after estimating and before wallet submission. This is a website transaction budget, not an onchain rule. Wallet edits, smart-account wrapping and transactions created by other clients can change the actual envelope and fee. Display the final wallet fee for confirmation.

Checks: purchase budget boundary tests, test/formal transaction tests, player UI tests (70 passing), contract lifecycle tests (5 passing), production build.
