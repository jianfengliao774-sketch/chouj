...
新版部署入口为 `/deploy-sparkdraw.html`：一个 drand 验签合约，加 0.1 / 5 / 10 / 50 / 100 BEM 五个场次。`contracts/drand-candidate/README.md` 记录测试、部署方式和待接入项目。部署记录接口为 `/api/sparkdraw/deployments`，必须取得真实已确认部署交易后才能更新玩家页面的地址、ABI和机制说明；不能仅替换展示地址而继续使用旧合约的购买逻辑。部署页和完整编译输入已准备，链上部署仍需指定钱包确认。
